#!/usr/bin/env python3
"""
run_accident.py — OpenCV live accident detection.

Usage:
    python run_accident.py                                     # default webcam
    python run_accident.py --cam 1                             # second camera
    python run_accident.py --cam "rtsp://192.168.1.10:554/s"   # RTSP stream
"""
...
WINDOW_TITLE = "Argus — Accident Detection  [q] quit"
...
def run(source: int | str = 0, conf: float = 0.40) -> None:
    ...
    try:
        while True:
            ...
            for e in events:
                log.warning("🚨 ACCIDENT DETECTED! (time: %s)", e["time"])
...
def parse_args():
    p = argparse.ArgumentParser(description="Argus Accident detection")
