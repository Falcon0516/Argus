#!/usr/bin/env python3
"""
dashboard.py — Streamlit dashboard for Accident detection.
...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Argus — Traffic Accident Detection",
    page_icon="💥",
    layout="wide",
)
...
def _worker(video_source: int | str, conf: float, state: dict) -> None:
    ...
    try:
        detector = AccidentDetector(conf=conf)
    ...
    while True:
        ...
        with lock:
            ...
            if events:
                ...
                # Prevent spamming the log for the same continuous accident
                if time.time() - last_accident_time > 5.0:
                    for e in events:
                        state["log"].appendleft(e)
                        state["total_count"] += 1
                    last_accident_time = time.time()
            else:
                if time.time() - last_accident_time > 3.0:
                    state["status"] = "SAFE"
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption("Argus — Road Safety Monitor")
        st.divider()
        ...
        conf_val = st.slider("Vehicle Confidence", 0.10, 0.90, 0.40, 0.05)
        st.divider()
    ...
    st.markdown("## 💥 Road Traffic Accident Monitor")

    cols = st.columns(3)
    metrics = [
        ("FPS",           f"{fps:.1f}",  "#00E6FF"),
        ("Status",        status,        "#FF3333" if status == "ACCIDENT DETECTED" else "#32FF32"),
        ("Total Collisions", total_cnt,  "#FF3333"),
    ]
    ...
    with log_col:
        ...
        st.markdown(html or "<div style='color:#444;margin-top:20px'>No collisions detected…</div>",
                    unsafe_allow_html=True)
