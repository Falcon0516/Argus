#!/usr/bin/env python3
"""
dashboard.py — Streamlit dashboard for waste and litter detection.
...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Argus — Waste & Litter Detection",
    page_icon="🗑️",
    layout="wide",
)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption("Argus — Litter Monitor")
        st.divider()
        ...
        conf_val = st.slider("Confidence", 0.10, 0.80, 0.25, 0.05)
        st.divider()
    ...
    st.markdown("## 🗑️ Waste & Litter Monitor")

    cols = st.columns(3)
    metrics = [
        ("FPS",           f"{fps:.1f}",  "#00E6FF"),
        ("Status",        status,        "#FF9900" if status == "LITTER DETECTED" else "#32FF32"),
        ("Total Objects Detected", total_cnt, "#FF9900"),
    ]
    ...
    with log_col:
        st.markdown("#### 📋 Litter Detection Log")
        html = ""
        for entry in log_items[:50]:
            ...
            html += (
                ...
                f"border:1px solid {fg}'>LITTER</span>  "
                f"<span style='color:#ccc'>{entry['type']} ({entry['conf']:.2f})</span>  "
                f"</div>"
            )
        st.markdown(html or "<div style='color:#444;margin-top:20px'>No litter detected…</div>",
                    unsafe_allow_html=True)
