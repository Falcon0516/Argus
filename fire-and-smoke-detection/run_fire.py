#!/usr/bin/env python3
"""
run_fire.py — OpenCV live fire and smoke detection.

Usage:
    python run_fire.py                                     # default webcam
    python run_fire.py --cam 1                             # second camera
    python run_fire.py --cam "rtsp://192.168.1.10:554/s"   # RTSP stream
"""
...
WINDOW_TITLE = "Argus — Fire & Smoke Detection  [q] quit"


def run(source: int | str = 0, conf: float = 0.35) -> None:
    ...
    try:
        while True:
            ...
            # Red flashing border if fire/smoke detected
            if events:
...
def parse_args():
    p = argparse.ArgumentParser(description="Argus fire and smoke detection")
