#!/usr/bin/env python3
"""
dashboard.py — Streamlit dashboard for fire and smoke detection.
...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Argus — Fire & Smoke Detection",
    page_icon="🔥",
    layout="wide",
)
...
def _worker(video_source: int | str, conf: float, state: dict) -> None:
    ...
    try:
        detector = FireDetector(conf=conf)
        ...
        with lock:
            ...
            if events:
                ...
                for e in events:
                    ...
                    if e["type"] in state["totals"]:
                        state["totals"][e["type"]] += 1
            else:
                state["status"] = "SAFE"
...
def main() -> None:
    ...
    with st.sidebar:
        st.markdown("## 🔥 Fire & Smoke")
        st.caption("Argus — Hazard Monitor")
        st.divider()
...
- ⚪ Grey box — **SMOKE**
    ...
    st.markdown("## 🔥 Fire & Smoke Hazard Monitor")

    cols = st.columns(4)
    metrics = [
        ("FPS",           f"{fps:.1f}",  "#00E6FF"),
        ("Status",        status,        "#FF3333" if status == "DANGER" else "#32FF32"),
        ("Fire Alerts",   totals["FIRE"], "#FF3333"),
        ("Smoke Alerts",  totals["SMOKE"], "#a0a0a0"),
    ]
    ...
    with log_col:
        ...
        for entry in log_items[:50]:
            ...
            html += (
                ...
                f"<span style='color:#ccc'>Conf: {entry['conf']:.2f}</span>  "
                f"</div>"
            )
        st.markdown(html or "<div style='color:#444;margin-top:20px'>No hazards detected…</div>",
                    unsafe_allow_html=True)
