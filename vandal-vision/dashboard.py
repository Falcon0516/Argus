#!/usr/bin/env python3
"""
dashboard.py — Streamlit dashboard for VandalVision detection.
...
def _worker(video_source: int | str, state: dict) -> None:
    lock = state["lock"]
    ...
    while True:
        ...
        with lock:
            ...
            if events:
                state["status"] = "VANDALISM DETECTED"
                for e in events:
...
def main() -> None:
    ...
    with st.sidebar:
        st.markdown("## 🚫 VandalVision")
        st.caption("Argus — Vandalism Monitor")
        st.divider()
    ...
    st.markdown("## 🚫 VandalVision Monitor")

    cols = st.columns(3)
    metrics = [
        ("FPS",           f"{fps:.1f}",  "#00E6FF"),
        ("Status",        status,        "#FF3333" if status == "VANDALISM DETECTED" else "#32FF32"),
        ("Total Alerts",  total_cnt,     "#FF3333"),
    ]
