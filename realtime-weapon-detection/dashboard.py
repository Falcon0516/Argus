#!/usr/bin/env python3
"""
dashboard.py — Streamlit dashboard for real-time weapon detection.
...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Argus — Weapon Detection",
    page_icon="🔫",
    layout="wide",
)
...
def _worker(video_source: int | str, conf: float, state: dict) -> None:
    ...
    while True:
        ...
        with lock:
            ...
            if events:
                state["status"] = "WEAPON DETECTED"
                for e in events:
                    state["log"].appendleft(e)
                    state["total_count"] += 1
                
                # Audio alarm check
                if time.time() - state["last_alarm"] > 3.0:
                    play_alarm()
                    state["last_alarm"] = time.time()
            else:
                state["status"] = "SAFE"
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption("Argus — Security Monitor")
        st.divider()
        ...
        conf_val = st.slider("Confidence", 0.10, 0.90, 0.45, 0.05)
        st.divider()
    ...
    st.markdown("## 🔫 Weapon & Threat Monitor")

    cols = st.columns(3)
    metrics = [
        ("FPS",           f"{fps:.1f}",  "#00E6FF"),
        ("Status",        status,        "#FF3333" if status == "WEAPON DETECTED" else "#32FF32"),
        ("Total Threats", total_cnt,     "#FF3333"),
    ]
    ...
    with log_col:
        st.markdown("#### 📋 Security Alert Log")
        html = ""
        ...
        st.markdown(html or "<div style='color:#444;margin-top:20px'>No threats detected…</div>",
                    unsafe_allow_html=True)
